...
# ------------------------------------------------------------- 
    np.save('boundary_conditions/init_val_u.npy',values_u)
    np.save('boundary_conditions/init_val_v.npy',values_v)
    np.save('boundary_conditions/init_val_w.npy',values_w)
    np.save('boundary_conditions/init_val_p.npy',values_p)
    break
...
# ------------------ Possion equation -------------------------    
    ...
    if mgsolver == True:
        for multi_grid in range(multi_itr):
            ...
            np.save('residuals/naiver_stokes_res.npy',r)
