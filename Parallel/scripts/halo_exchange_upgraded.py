...
class HaloExchange:
    ...
    def mpi_init(self, proc_grid_dim, is_periodic, is_reordered) -> None:
        ...
        # TODO: Other topologies we need here
    ...
    def initialization(self, mesh, is_periodic=False, is_reordered=False):
        ...
        # print('Cartesian topology dimension: {}'.format(proc_grid_dim))
    ...
    def structured_halo_update_3D(self,input_cube):
        ...
        requests.append(self.comm.Isend(sendbuffer_1, dest=self.neighbors[self.FRONT]))
        requests.append(self.comm.Isend(sendbuffer_2, dest=self.neighbors[self.BEHIND]))
        requests.append(self.comm.Irecv(recvbuffer_1, source=self.neighbors[self.BEHIND]))
        requests.append(self.comm.Irecv(recvbuffer_2, source=self.neighbors[self.FRONT]))
        MPI.Request.Waitall(requests)
        requests.clear()
        ...
        requests.append(self.comm.Isend(sendbuffer_1, dest=self.neighbors[self.LEFT]))
        requests.append(self.comm.Isend(sendbuffer_2, dest=self.neighbors[self.RIGHT]))
        requests.append(self.comm.Irecv(recvbuffer_1, source=self.neighbors[self.RIGHT]))
        requests.append(self.comm.Irecv(recvbuffer_2, source=self.neighbors[self.LEFT]))
        MPI.Request.Waitall(requests)
        requests.clear()
        ...
        requests.append(self.comm.Isend(sendbuffer_1, dest=self.neighbors[self.TOP]))
        requests.append(self.comm.Isend(sendbuffer_2, dest=self.neighbors[self.BOTTOM]))
        requests.append(self.comm.Irecv(recvbuffer_1, source=self.neighbors[self.BOTTOM]))
        requests.append(self.comm.Irecv(recvbuffer_2, source=self.neighbors[self.TOP]))
        MPI.Request.Waitall(requests)
        requests.clear()
