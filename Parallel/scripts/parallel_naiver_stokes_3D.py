...
# initilization
he = HaloExchange(structured=True, halo_size=1, tensor_used=True,
...
def boundary_condition_velocity(values_u, values_v, values_w, sub_nx):
    ...
    if neighbors[RIGHT] == -2:
        tempu[0, 1:-1, 1:-1, sub_nx-1,0].assign(tf.Variable(tf.ones((1, sub_nx, sub_nx)))[0, :]*ub)
        tempv[0, 1:-1, 1:-1, sub_nx-1,0].assign(tf.Variable(tf.zeros((1, sub_nx, sub_nx)))[0, :])
        tempw[0, 1:-1, 1:-1, sub_nx-1,0].assign(tf.Variable(tf.zeros((1, sub_nx, sub_nx)))[0, :])
    ...
    if neighbors[BEHIND] == -2:
        tempu[0, 1:-1, sub_nx-1, 1:-1, 0].assign(tf.Variable(values_u)[0, 1:-1, sub_nx-2, 1:-1, 0])
        tempv[0, 1:-1, sub_nx-1, 1:-1,0].assign(tf.Variable(tf.zeros((1, sub_nx, sub_nx)))[0, :])
        tempw[0, 1:-1, sub_nx-1, 1:-1 ,0].assign(tf.Variable(tf.zeros((1, sub_nx, sub_nx)))[0, :])
    ...
    # top bound
    if neighbors[TOP] == -2:
        tempu[0, sub_nx-1, 1:-1, 1:-1, 0].assign(tf.Variable(values_u)[0, sub_nx-2, 1:-1, 1:-1, 0])
        tempv[0, sub_nx-1, 1:-1, 1:-1, 0].assign(tf.Variable(values_v)[0, sub_nx-2, 1:-1, 1:-1, 0])
        tempw[0, sub_nx-1, 1:-1, 1:-1,0].assign(tf.Variable(tf.zeros((1, sub_nx, sub_nx)))[0, :])
...
def boundary_condition_pressure(values_p, sub_nx):
    ...
    # left right
    if neighbors[RIGHT] == -2:
        tempp[0, 1:-1, 1:-1, sub_nx-1,0].assign(tf.Variable(tf.zeros((1, sub_nx, sub_nx)))[0, :])
    if neighbors[LEFT] == -2:
    ...
    if neighbors[BEHIND] == -2:
        tempp[0, 1:-1, sub_nx-1, 1:-1, 0].assign(tf.Variable(values_p)[0, 1:-1, sub_nx - 2, 1:-1, 0])

    # bottom top
    if neighbors[BOTTOM] == -2:
        tempp[0, 1, 1:-1, 1:-1, 0].assign(tf.Variable(values_p)[0, 2, 1:-1, 1:-1, 0])
    if neighbors[TOP] == -2:
        tempp[0, sub_nx-1, 1:-1, 1:-1, 0].assign(tf.Variable(values_p)[0, sub_nx-2, 1:-1, 1:-1, 0])
...
def boundary_condition_source(b, nx):
    tempb = tf.Variable(b)
    if neighbors[LEFT] == -2:
        tempb[0, 1:-1, 1:-1, 1, 0].assign(tf.Variable(b)[0, 1:-1, 1:-1, 2, 0])
    return tempb
...
# ------------------ Numerical set up ----------------------
multi_itr = 1        # Iterations of multi-grid
...
# ------------------ Boundary conditions ----------------------
    ...
    np.save('boundary_conditions/init_p_val_u_{}.npy'.format(rank),values_u)
    np.save('boundary_conditions/init_p_val_v_{}.npy'.format(rank),values_v)
    np.save('boundary_conditions/init_p_val_w_{}.npy'.format(rank),values_w)
    np.save('boundary_conditions/init_p_val_p_{}.npy'.format(rank),values_p)
...
# ------------------ Possion equation -------------------------
    ...
    b = tf.reshape(b[0,1:-1,1:-1,1:-1,0],(1,sub_nx,sub_ny,sub_ny,1))           # modify the shape
    
    print(f'values_p shape: {values_p.shape}')
    print(f'b shape: {b.shape}')

# ----------------------------------------------------------------------------------------------------------------------------------

    multi_grid_start = perf_counter()  # multi-grid time counter
    # Multi-grid method is dominant
    if mgsolver == True:
        for multi_grid in range(multi_itr):
            ...
            np.save('parallel_residuals/parallel_naiver_stokes_res_{}'.format(rank),r)
            ...
            for Jacobi in range(j_itr):
                ...
                w_64 = w_64 - temp/w5[0, 1, 1, 1, 0]+ r/w5[0, 1, 1, 1, 0]
...
# print(f'resultant values_p shape: {values_p.shape}')
