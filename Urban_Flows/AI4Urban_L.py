...
# output   
        ...
        if save_fig == True and itime % n_out == 0:
            np.save("Urban_data_L/w"+str(itime), arr=values_w.cpu().detach().numpy()[0,0,:,:])
            np.save("Urban_data_L/v"+str(itime), arr=values_v.cpu().detach().numpy()[0,0,:,:])
            np.save("Urban_data_L/u"+str(itime), arr=values_u.cpu().detach().numpy()[0,0,:,:])
