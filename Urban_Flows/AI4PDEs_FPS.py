...
k1 = torch.ones(input_shape, device=device)*0.4*dx**2/dt
...
# ***** Parabolic inflow *****
parabolic_v = ((1 - 4*((Y-2)/4)**2) * (1 - 4*((Z-2)/4)**2) * ub)
parabolic_v = torch.from_numpy(parabolic_v).to(device) 
# ***** Parabolic inflow *****
#######################################################
...
if LIBM == True:
    ...
    sigma_bc = torch.zeros(input_shape, dtype=torch.float32, device=device) 
    for i in range(nx):
        for j in range(ny):
            for k in range(nz):
...
plt.imshow(sigma.cpu().detach().numpy()[0,0,160,:,:])
plt.colorbar()
...
# Restart simulation 
if Restart == True:
    values_u = np.load(save_path+'/u'+str(nrestart)+'.npy').astype('float32')
    values_v = np.load(save_path+'/v'+str(nrestart)+'.npy').astype('float32')
    values_w = np.load(save_path+'/w'+str(nrestart)+'.npy').astype('float32')
    values_p = np.load(save_path+'/p'+str(nrestart)+'.npy').astype('float32')
    values_u = torch.tensor(values_u, device=device)
    values_v = torch.tensor(values_v, device=device)
    values_w = torch.tensor(values_w, device=device)
    values_p = torch.tensor(values_p, device=device)
...
###############################################################
    def boundary_condition_u(self, values_u, values_uu, parabolic_v):
        nz = values_u.shape[2]
        ny = values_u.shape[3]
        nx = values_u.shape[4]
...
# ***** Parabolic inflow *****
        values_uu[0,0,:,:,0] = parabolic_v
        values_uu[0,0,:,:,nx+1] = parabolic_v
# ***** Parabolic inflow *****
    ...
    def log_law_boundary_condition(self, values_u, sigma_bc):
        values_u = torch.where(torch.eq(sigma_bc,1),values_u / (1 + dt * 4.9e-03 * torch.abs(values_u) / dx), values_u)
        return values_u
    ...
    def PG_vector(self, values_uu, values_vv, values_ww, values_u, values_v, values_w, k1, k_uu, k_vv, k_ww, ADx_u, ADy_u, ADz_u, ADx_v, ADy_v, ADz_v, ADx_w, ADy_w, ADz_w, AD2_u, AD2_v, AD2_w):
        ...
        k_u = 1 * torch.ones(input_shape, device=device)
        k_v = 1 * torch.ones(input_shape, device=device)
        k_w = 1 * torch.ones(input_shape, device=device)
    ...
    def forward(self, values_u, values_uu, values_v, values_vv, values_w, values_ww, values_p, values_pp, b_uu, b_vv, b_ww, k1, dt, iteration, k_uu, k_vv, k_ww, parabolic_v):
    # Solid body
        if LIBM == True: [values_u, values_v, values_w] = self.solid_body(values_u, values_v, values_w, sigma, dt)
#  ***** log law wall *****
        values_u = self.log_law_boundary_condition(values_u, sigma_bc)
#  ***** log law wall *****
    # Padding velocity vectors 
        values_uu = self.boundary_condition_u(values_u,values_uu,parabolic_v)   # ****************************** halo update -> values_uu ************************  
...
#  ***** log law wall *****
        b_u = self.log_law_boundary_condition(b_u, sigma_bc)
#  ***** log law wall *****
    # Padding velocity vectors 
        b_uu = self.boundary_condition_u(b_u,b_uu,parabolic_v)  # ****************************** halo update -> b_uu ************************ 
...
with torch.no_grad():
    for itime in range(1,ntime+1):
        [values_u, values_v, values_w, values_p, w, r] = model(values_u, values_uu, values_v, values_vv, values_w, values_ww, values_p, values_pp, b_uu, b_vv, b_ww, k1, dt, iteration, k_uu, k_vv, k_ww, parabolic_v)
# output   
        ...
        if itime == ntime:
            np.save(save_path+"/p"+str(itime), arr=values_p.cpu().detach().numpy())              
            np.save(save_path+"/w"+str(itime), arr=values_w.cpu().detach().numpy())
            np.save(save_path+"/v"+str(itime), arr=values_v.cpu().detach().numpy())
            np.save(save_path+"/u"+str(itime), arr=values_u.cpu().detach().numpy())      
    end = time.time()
